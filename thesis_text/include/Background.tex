...
At the moment of writing reinforcement learning is rapidly evolving, but it is plagued
by problems which prevent it from widespread adoption in practical applications
like robot control.
Here we list the most fundamental problems:
\begin{itemize}
		\item high random seed dependence
		\item high variance of final algorithm performance across different tasks ---
		no algorithm is universally better than all others at every problem,
		despite the aim toward generality
\item sensitivity to hyperparameters
\item brittleness of solutions to changes in the environment
\end{itemize}

These problems also result in ambiguities in research, as discussed in
\cite{drlthatmatters}.
